### Запуск контейнера:
IMAGE ID: 5a1eca57fa24

Для запуска собранного образа, в рамках дз, необходимо выполнить следующую строчку:

```terminal
docker run -d -p 8000:8000 5a1eca57fa24
```
После запуска контейнера, его работоспособность можно проверить: http://127.0.0.1:8000/ping/

В случае, если образ, по-указанному выше ID недоступен, необходимо собрать его локально, выполнив команду:
```Terminal
docker build -t fastapi-img:1.0 .
```

После чего можно запустить контейнер по заданному имени:
```terminal
docker run -d -p 8000:8000 fastapi-img:1.0
```
